...
<template>
    <div>
        ...
        <Button type="primary" @click="addObj">添加组</Button>
        </div>
        ...
        <Modal v-model="showModal" :closable='false' :mask-closable='false' :width="600">
            ...
            >
                <FormItem v-for="(item, i) in form" :key="i" :label="item.name + ':'" :prop="item.key">
                    ...
                    <div v-if="item.classParent === 'form-row-inputs'" :class="item.classParent">
                        <Input v-model="modalForm[item.key]"></Input> .
                        <Input v-model="modalForm[item.key]"></Input> .
                        <Input v-model="modalForm[item.key]"></Input> .
                        <Input v-model="modalForm[item.key]"></Input> :
                        <Input v-model="modalForm[item.key]"></Input>
            ...
            <div slot="footer">
                ...
                <Button v-if="isSaveBtn" type="primary" :loading="modal.saveModalLoading" @click="saveModal">提交</Button>
        ...
        saveModal () {
            ...
            this.$refs['modalForm'].validate((valid) => {
                if (valid) {
                    ...
                    debugger
                    let children = [];
                    if (typeof this.specialObj !== 'undefined') {
                        children = this.specialObj.children || []
                    }
                    let item = {
                        id: '12',
                        parentID: typeof this.specialObj !== 'undefined' ? this.specialObj.id : 0,
                    ...
                    if (typeof this.specialObj !== 'undefined') {
                        ...
                        console.log(JSON.stringify(this.treeList, null, 2))
                    }
                    setTimeout(() => {
